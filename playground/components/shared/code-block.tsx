...
export function CodeBlock({
  ...
  const highlighted = useMemo(() => {
    ...
    return lines.map((line, i) => {
      const clean = minIndent !== Infinity ? line.slice(minIndent) : line;

      return (
        <div key={i} className="leading-relaxed whitespace-pre font-mono text-sm">
          {showLineNumbers && (
  ...
  return (
    <div className={cn('font-mono text-sm overflow-x-auto rounded-lg', className)}>
      {highlighted}
    </div>
  );
}
